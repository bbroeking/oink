...
describe("open/submit lifecycle transitions", () => {
  test("submission transitions distinguish in-flight from uncertain", () => {
    const submitting = digSessionReducer(initialDigSessionState, {
      type: "submission_started",
    });
    expect(submitting.submissionStatus).toBe("submitting");
    const uncertain = digSessionReducer(submitting, {
      type: "submission_uncertain",
    });
    expect(uncertain.submissionStatus).toBe("uncertain");
    expect(
      digSessionReducer(uncertain, { type: "submission_settled" })
        .submissionStatus,
    ).toBe("idle");
  });
  ...
  test("open_no_crew raises the crewless flag; opened without clearNoCrew leaves it", () => {
    const crewless = digSessionReducer(initialDigSessionState, {
      type: "open_no_crew",
    });
...
describe("incident — reconcile debounce + server-truth decision", () => {
  ...
  test("reconciled not-dug retires a MATCHING stale local claim", () => {
    const stale: DigSessionState = {
      ...initialDigSessionState,
      dugWindow: 555,
    };
    const next = digSessionReducer(stale, {
      type: "reconciled",
      dug: false,
      window: 555,
    });
  ...
  test("reconciled not-dug leaves a claim for a DIFFERENT window alone", () => {
    const other: DigSessionState = {
      ...initialDigSessionState,
      dugWindow: 900,
    };
    const next = digSessionReducer(other, {
      type: "reconciled",
      dug: false,
      window: 555,
    });
