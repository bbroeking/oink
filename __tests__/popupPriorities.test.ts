...
const SLOT_SOURCE_FILES = [
  ...
  "components/habitat/HabitatExpansionDiscovery.tsx",
];
...
function callSitesIn(file: string): CallSite[] {
  ...
  while ((m = open.exec(src)) !== null) {
    ...
    const priorityExpr = args[args.length - 1].trim();
    const literalId = args[0].match(/^\s*"([^"]+)"\s*$/)?.[1];
    // A call may scope its runtime slot id by render mode while deliberately
    // sharing one canonical priority (Barn's compact Interior fallback does
    // this). In that case the priority member is the registry id this guard
    // must account for. Require the conditional to retain that exact literal,
    // so an unrelated dynamic expression cannot masquerade as a wired slot.
    const priorityId = priorityExpr.match(
      /^POPUP_PRIORITIES\.([A-Za-z_$][\w$]*)$/,
    )?.[1];
    const id = literalId ?? priorityId;
    if (!id) continue;
    if (!literalId) {
      const quotedCanonical = new RegExp(
        `["']${id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}["']`,
      );
      if (!quotedCanonical.test(args[0])) continue;
    }
    sites.push({
      id,
      priorityExpr,
