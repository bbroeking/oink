...
describe("generateBoard", () => {
  ...
  test("board shape: counts, bounds, no overlaps", () => {
    for (const seed of [1, 42, 999983, 2147483646, 1234567]) {
      ...
      const all = board.cells.map((c, i) => (c ? i : -1)).filter((i) => i >= 0);
  ...
  test("domino cells are adjacent; L cells form an L", () => {
    ...
    const dr = Math.abs(
      Math.floor(a / PATCH_COLS) - Math.floor(b / PATCH_COLS),
    );
...
describe("generateBoard — unique relic placement", () => {
  ...
  test("board.finds carries 'unique' ONLY when a relic is present", () => {
    ...
    expect(withUnique.unique).toEqual({
      id: "old_boot",
...
describe("feeding windows", () => {
  ...
  test("feedingCountdown formats h/m", () => {
    ...
    expect(feedingCountdown(boundary - 2 * 3600000 - 10 * 60000)).toBe(
      "2h 10m",
    );
...
describe("persisted-zone commuter windows", () => {
  const localSchedule = {
    mode: "commuter_local" as const,
    windowSecs: 28800,
    openSecs: 14400,
    offsetSecs: 0,
  };

  afterEach(() => resetFeedingTimeZoneSession());

  const at = (zone: string, day: number, hour: number, minute = 0) =>
    zonedCivilToEpochMs({ year: 2026, month: 7, day, hour, minute }, zone);

  test("uses exactly the 08–12, 16–20, and 00–04 civil windows", () => {
    const zone = "America/Los_Angeles";
    applyEffectiveFeedingTimeZone(zone);
    const expected = [
      [16, 8, true],
      [16, 12, false],
      [16, 16, true],
    ...
    for (const [day, hour, open] of expected) {
      expect(patchPhaseOpen(at(zone, day, hour), localSchedule)).toBe(open);
    }
  });

  test("midnight belongs to the prior 08:00 dig-day and 08:00 rolls it", () => {
    const zone = "Asia/Tokyo";
    applyEffectiveFeedingTimeZone(zone);
    const midnight = windowIndex(at(zone, 17, 0), localSchedule);
    expect(windowIndex(at(zone, 17, 7, 59), localSchedule)).toBe(midnight);
    expect(patchPhaseOpen(at(zone, 17, 7, 59), localSchedule)).toBe(false);
    expect(windowIndex(at(zone, 17, 8), localSchedule)).toBe(midnight + 1);
    expect(patchPhaseOpen(at(zone, 17, 8), localSchedule)).toBe(true);
  });

  test("decodes every namespaced id to its next civil boundary", () => {
    const zone = "Asia/Kolkata";
    applyEffectiveFeedingTimeZone(zone);
    const starts = [at(zone, 16, 8), at(zone, 16, 16), at(zone, 17, 0)];
    const ends = [at(zone, 16, 16), at(zone, 17, 0), at(zone, 17, 8)];
    starts.forEach((start, index) => {
      expect(
        windowEndsAtMs(windowIndex(start, localSchedule), localSchedule),
      ).toBe(ends[index]);
    });
  });

  test("local ids use a collision-proof namespace and match by civil date", () => {
    const ids = [
      "America/New_York",
      "America/Los_Angeles",
      "Asia/Tokyo",
      ...
      applyEffectiveFeedingTimeZone(zone);
      return windowIndex(at(zone, 16, 8), localSchedule);
    });
    expect(new Set(ids)).toEqual(new Set([ids[0]]));
    expect(ids[0]).toBeGreaterThanOrEqual(DIG_LOCAL_WINDOW_ID_OFFSET);
    const instant = at("America/New_York", 16, 8);
    expect(ids[0]).not.toBe(
      windowIndex(instant, { ...localSchedule, mode: "uniform" }),
    );
    expect(ids[0]).not.toBe(
      windowIndex(instant, { ...localSchedule, mode: "commuter_eastern" }),
    );
  });

  test("spring DST keeps midnight and 04:00 as civil boundaries", () => {
    const zone = "America/New_York";
    applyEffectiveFeedingTimeZone(zone);
    const midnight = zonedCivilToEpochMs(
      { year: 2026, month: 3, day: 8, hour: 0, minute: 0 },
      zone,
    );
    const four = zonedCivilToEpochMs(
      { year: 2026, month: 3, day: 8, hour: 4, minute: 0 },
      zone,
    );
    expect(phaseClosesAtMs(midnight, localSchedule)).toBe(four);
    expect(four - midnight).toBe(3 * 3600000);
    expect(patchPhaseOpen(four - 1, localSchedule)).toBe(true);
    expect(patchPhaseOpen(four, localSchedule)).toBe(false);
    expect(
      zonedCivilParts(
        windowEndsAtMs(windowIndex(midnight, localSchedule), localSchedule),
        zone,
      ),
    ).toMatchObject({
      month: 3,
...
describe("crewBoardSeed — seeded crew boards (window × group)", () => {
  ...
  test("different crew → different seed → different board (same window)", () => {
    ...
    expect(
      JSON.stringify(generateBoard(crewBoardSeed(402, crewA))),
    ).not.toEqual(JSON.stringify(generateBoard(crewBoardSeed(402, crewB))));
  });
...
describe("tileIndexAt — border-box touch → tile index", () => {
  ...
  test("center of a few tiles maps to the right index", () => {
    ...
    expect(
      tileIndexAt(INSET + 2 * TILE + 10, INSET + 1 * TILE + 10, TILE, INSET),
    ).toBe(1 * PATCH_COLS + 2);
...
describe("Eastern DST", () => {
  test("06:00 Eastern follows the spring DST offset", () => {
    ...
    expect(new Date(nextOpenAtMs(before)).toISOString()).toBe(
      "2026-03-07T17:00:00.000Z",
    );
    expect(new Date(nextOpenAtMs(after)).toISOString()).toBe(
