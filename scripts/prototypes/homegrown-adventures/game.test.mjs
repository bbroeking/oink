...
test("the earned crop choice keeps Rosie's current Adventure purpose attached to both harvests", () => {
	...
	assert.match(appSource, /crop-choice-layout-\$\{variant\}/);
	assert.match(stylesSource, /\.crop-choice-art img \{/);
	assert.match(stylesSource, /\.crop-choice-tiles \{ grid-template-columns: repeat\(2,minmax\(0,1fr\)\)/);
	assert.match(stylesSource, /\.crop-choice-tray \{/);
	assert.match(stylesSource, /\.crop-choice-rows \{/);
