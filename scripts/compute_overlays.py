...
# Slop Club members-only catalog (emoji column is NULL — real art, no glyph).
MEMBERS_SQL = f"{REPO}/supabase/migrations/20260690000000_seed_members_catalog.sql"
...
REL_FILES = (
    f"{REPO}/constants/hat_rel.generated.ts",
    f"{REPO}/constants/membersRel.generated.ts",
)
# Categories that never read a per-item HAT_OVERLAYS row at render time:
# full-canvas (resolveSlot uses CATEGORY_OVERLAYS) + tickle_particle (a burst
# FX, never worn on the pig). compute_overlays should not emit dead rows for them.
NON_OVERLAY_CATEGORIES = {"aura", "background", "tickle_particle"}


def relspec_ids():
    """Ids that have an anchor-relative RelSpec (hat_rel + membersRel). These
    render via the RelSpec path, so their HAT_OVERLAYS row is dead — skip it."""
    ids = set()
    for p in REL_FILES:
        try:
            src = open(p).read()
            ...
            continue
        ids |= set(re.findall(r"^\t(\w+):\s*\{\s*pivot:", src, re.M))
    return ids
...
def parse_catalog():
    ...
    items = {}
    for sqlpath in (CATALOG_SQL, MUDWAR_SQL, MEMBERS_SQL):
        try:
            fh = open(sqlpath)
        except FileNotFoundError:
            continue
        with fh:
            for line in fh:
                ...
                # or the members seed: ('id', 'Name', NULL, 5500, 1000, 'hat', ...).
                # 3rd field (emoji) may be a quoted string OR NULL; tolerant of
                # leading indentation.
                m = re.match(
                    r"^\s*\('([^']+)',\s*'[^']*',\s*(?:'[^']*'|NULL),\s*\d+,\s*\d+,\s*'([^']+)',",
                    line,
                )
                if m:
...
def main():
    ...
    flags = []
    skipped = []
    migrated = []  # items now on the RelSpec path — no overlay emitted
    ...
    for asset_dir, fname in (
        ...
        if not category:
            ...
            continue
        # Skip items that render via the anchor-relative RelSpec path (the
        # canonical model — tools/placement_studio), and categories that never
        # read a per-item overlay. Leaves only genuinely-legacy worn items, so
        # this file shrinks to the real migration backlog instead of ~117 dead
        # rows the renderer ignores.
        if item_id in has_rel:
            migrated.append(f"{item_id} ({category})")
            continue
        if category in NON_OVERLAY_CATEGORIES:
            skipped.append(f"{item_id}: {category} (no per-item overlay)")
            continue
