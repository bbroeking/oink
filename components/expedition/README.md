...
4. **Wall fight** — tickles charge Zoomies (shown as a captionless meter and sprite energy, never a number); at full charge she bursts. Play the tucked card once per fight. She is never hurt — only stalled, warmly.
...
| **Components** (this directory) | `JournalHome` (send-off + road), `RoadMap`, `GearRack`, `CardHand`, `StatPips`, `PostcardModal`, `ScuffleView` (fight), `TrainingSheet`, `BestiaryShelf`, `RosiePose`/`RosieCharged`, `EnemySilhouette` (hand-drawn ink SVGs), `ZoomiesMeter` (`ProgressTrack`, captionless — the charge is spoken in words, never a number), `Ceremony` (shared spring+Tape entrance) |
...
- **Feelings shown, never numbered:** Zoomies is sprite energy plus a captionless meter; only progression numbers (HP, stats, jar) are digits.
- **Taste law:** tokens only, design-system primitives (`Sticker`/`ListRow`/`Sheet`/`Chip`/`ProgressTrack`/`Stat`/`T`), zero emoji, hard zero-blur shadows, all motion through `useMotionPolicy`. Zero `ttp/*` lint warnings (2026-09-11 conformance pass).
