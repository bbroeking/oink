...
import type { PigAnimation, PigFacing } from "@/components/ui/pigRendererContract";
import { PIG_ANIMATION_SPECS, resolveFacingAnimation } from "@/components/ui/pigRendererContract";
...
const VIEWS: readonly ViewDef[] = [
	...
	{ key: "front_sit", label: "Front sit", col: "F·sit", animation: "sit", frame: 0 },
	{ key: "turn_r", label: "Turn R", col: "Turn R", animation: "idle", facing: "right", frame: 0 },
	{ key: "turn_l", label: "Turn L", col: "Turn L", animation: "idle", facing: "left", frame: 0 },
	{ key: "turn_sit", label: "Turn L sit", col: "T·sit", animation: "sit", facing: "left", frame: 0 },
	...
	{ key: "surprise", label: "Surprise", col: "Surpr.", animation: "surprise", frame: 1 },
];
...
function slotOf(id: string, anchor: string | undefined): Slot {
	if (anchor === "eyes" || anchor === "eye_l" || anchor === "eye_r") return "glasses";
...
const ITEMS: readonly AuditItem[] = Object.keys(HAT_IMAGES)
	.filter((id) => HAT_REL[id] && !NON_COSMETIC.has(id) && !id.startsWith("flag_"))
	.sort()
	.map((id) => {
		const anchor = HAT_REL[id]?.anchor ?? "head";
		return { id, slot: slotOf(id, anchor), anchor, hasSide: id in HAT_SIDE_IMAGES };
	});

function equippedProps(item: AuditItem) {
	const eq: EquippedItem = { id: item.id, category: item.slot, emoji: null };
	switch (item.slot) {
		case "hat": return { equipped: eq };
		case "bow": return { equippedBow: eq };
		case "glasses": return { equippedGlasses: eq };
		case "neck": return { equippedNeck: eq };
		case "held": return { equippedHeld: eq };
		case "aura": return { equippedAura: eq };
	}
}
...
const MiniStage = memo(function MiniStage({
	...
	const scale = size / ART_SIZE.stage;
	return (
...
const ItemRow = memo(function ItemRow({
	...
	return (
		<Sticker color={flag ? "sun" : "paper"} rotate={0} radius={RADII.md} style={styles.row}>
			<View style={styles.rowHead}>
				<T role="cardTitleSm" numberOfLines={1} style={styles.rowTitle}>
					{item.id}
				</T>
				<Tag label={SLOT_LABELS[item.slot]} tone="paper" />
				<Tag label={item.hasSide ? "side art" : "front only"} tone={item.hasSide ? "sage" : "paper"} />
				{flag ? <Tag label={`flag · ${flag.views.length}`} tone="rose" /> : null}
			</View>
				...
				{views.map((view) => {
					...
					return (
						<Pressable
							...
							style={[styles.cell, { width: cell }, flagged && styles.cellFlagged]}
						>
...
function PigAngleAudit() {
	...
	const [viewKeys, setViewKeys] = useState<string[]>(DEFAULT_VIEW_KEYS);
	const [flags, setFlags] = useState<Flags>({});
	const [focus, setFocus] = useState<{ item: AuditItem; view: ViewDef } | null>(null);

	useEffect(() => {
		AsyncStorage.getItem(FLAGS_KEY)
			.then((raw) => { if (raw) setFlags(JSON.parse(raw) as Flags); })
			.catch(() => {});
	}, []);
	...
	const views = useMemo(() => VIEWS.filter((v) => viewKeys.includes(v.key)), [viewKeys]);
	const items = useMemo(
		() =>
			ITEMS.filter(
				(it) =>
		...
		[slot, sideOnly, flaggedOnly, flags],
	);
	...
	const cell = Math.max(
		ART_SIZE.badge,
		Math.floor((rowInner - SPACE.xs * (views.length - 1)) / Math.max(1, views.length)),
	);

	const toggleView = (key: string) =>
		setViewKeys((keys) =>
			keys.includes(key)
				? keys.length > 1 ? keys.filter((k) => k !== key) : keys
				: VIEWS.filter((v) => v.key === key || keys.includes(v.key)).map((v) => v.key),
		);
	...
	const header = (
		<View style={styles.header}>
			...
			<Hand tone="secondary">
				{ITEMS.length} items · {sideCount} with side art · {flagCount} flagged · {items.length} shown
			</Hand>

			<View style={styles.chips}>
				{PIG_IDS.map((id) => (
					<Chip key={id} label={pigDefinition(id).name} selected={pigId === id} onPress={() => setPigId(id)} />
				))}
			</View>
			<View style={styles.chips}>
				<Chip label="All slots" selected={slot === null} onPress={() => setSlot(null)} />
				{SLOTS.map((s) => (
					<Chip key={s} label={SLOT_LABELS[s]} selected={slot === s} onPress={() => setSlot(s)} />
				))}
			</View>
			<View style={styles.chips}>
				<Chip label="Side art only" selected={sideOnly} onPress={() => setSideOnly((v) => !v)} />
				<Chip label="Flagged only" selected={flaggedOnly} onPress={() => setFlaggedOnly((v) => !v)} />
			</View>
			<View style={styles.chips}>
				{VIEWS.map((v) => (
					<Chip key={v.key} label={v.label} selected={viewKeys.includes(v.key)} onPress={() => toggleView(v.key)} />
				))}
			</View>

			<View style={[styles.columns, { gap: SPACE.xs }]}>
				{views.map((v) => (
					<T key={v.key} role="kickerPillSm" tone="secondary" align="center" numberOfLines={1} style={{ width: cell }}>
						{v.col}
					</T>
				))}
			</View>
	...
	const toggleFocusFlag = () => {
		...
		else next[focus.item.id] = { views: nextViews, note: prev?.note ?? "", at: new Date().toISOString() };
		saveFlags(next);
	};
	const setFocusNote = (note: string) => {
		...
		else next[focus.item.id] = { views: prev?.views ?? [], note, at: new Date().toISOString() };
		saveFlags(next);
	};
	...
	return (
		<View style={styles.page}>
			...
			<FlatList
				...
				renderItem={({ item }) => (
					<ItemRow item={item} pigId={pigId} views={views} cell={cell} flag={flags[item.id]} onOpen={(it, v) => setFocus({ item: it, view: v })} />
				)}
				ListHeaderComponent={header}
				ListFooterComponent={
					<View style={styles.footer}>
						<Button variant="gold" size="sm" onPress={copyReport} disabled={flagCount === 0}>
							Copy report
						</Button>
						<Button variant="ghost" size="sm" onPress={() => saveFlags({})} disabled={flagCount === 0}>
							Clear flags
						</Button>
					</View>
				}
			...
			<Sheet
				...
				kicker={focus ? `${SLOT_LABELS[focus.item.slot]} · ${focus.item.anchor} · ${focus.item.hasSide ? "side art" : "front only"}` : undefined}
				title={focus?.item.id}
				subtitle={focus ? `${pigDefinition(pigId).name} · ${focus.view.label}` : undefined}
			>
				{focus ? (
					<FocusBody
						focus={focus}
...
function FocusBody({
	...
	const [scrub, setScrub] = useState<{ view: ViewDef; frame: number } | null>(null);
	...
	return (
		<View style={styles.focus}>
			<View style={styles.focusStage}>
				<MiniStage item={focus.item} pigId={pigId} view={focus.view} size={ART_SIZE.stage} frame={frame} />
			</View>
			<View style={styles.chips}>
				{VIEWS.map((v) => (
					<Chip key={v.key} label={v.label} selected={v.key === focus.view.key} onPress={() => onView(v)} />
				))}
			</View>
			<View style={styles.chips}>
				{Array.from({ length: frames }, (_, i) => (
					<Chip key={i} label={`f${i + 1}`} selected={frame === i} onPress={() => setFrame(i)} />
				))}
			</View>
			<View style={styles.chips}>
				{PIG_IDS.map((id) => (
					<Chip key={id} label={pigDefinition(id).name} selected={pigId === id} onPress={() => onPig(id)} />
				))}
			</View>
			<Button variant={flagged ? "primary" : "gold"} onPress={onToggleFlag}>
				{flagged ? "Unflag this view" : "Flag this view"}
			</Button>
			<TextField label="Note" value={note} onChangeText={onNote} placeholder="what's wrong here" />
		</View>
	);
}
...
const styles = StyleSheet.create({
	...
	list: { paddingHorizontal: PAGE_PAD, paddingBottom: SPACE.xxl, gap: SPACE.md },
	header: { gap: SPACE.sm, marginBottom: SPACE.xs },
	chips: { flexDirection: "row", flexWrap: "wrap", gap: SPACE.xs },
	columns: { flexDirection: "row", paddingHorizontal: SPACE.card + BORDER.ink, marginTop: SPACE.xs },
	row: { gap: SPACE.sm },
	rowHead: { flexDirection: "row", alignItems: "center", flexWrap: "wrap", gap: SPACE.xs },
	...
	cellFlagged: { borderWidth: BORDER.heavy, borderColor: UI_COLORS.warningText },
	footer: { flexDirection: "row", justifyContent: "center", gap: SPACE.sm, marginTop: SPACE.lg },
