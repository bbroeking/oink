...
## Cosmetic art regeneration

`tools/regen_studio.py` (→ `http://127.0.0.1:8131/`) reviews + regenerates item art through the Codex CLI ImageGen lane (`codex exec --enable image_generation`, prompt via stdin — `-i` is variadic and eats positional prompts). Master-detail UI: OK / Redo-with-prompt / Fresh / Decline / Undo / Deprecate per item, inline drag-to-place (saves through placement studio's save-rel endpoint — single writer stays intact), optional Rosie-as-reference attach. The flat-sticker law for worn items: front silhouette only, no wrap-around/back-of-head parts; "flat" means the camera angle, never the rendering — revise prompts must be fidelity-first or they strip the art's character. Deprecations maintain a rebuild-all `..._deprecate_items.sql`. Needs placement studio (8124) running for placement saves.

## Cosmetic placement

`tools/placement_studio.py` (→ `http://127.0.0.1:8124/`) is the **single** tool for placing cosmetics on Rosie — item anchors (Items mode) and pig anatomy anchors (Pig mode). It auto-discovers every item, shows a live on-pig preview matching `PigStage.resolveSlot`, and autosaves: item RelSpecs → `constants/hat_rel.generated.ts` (rebuild-all, sorted), pig anchors → `PIG_FRAME_ANCHORS` in `constants/hats.ts` (and keeps `REST_ANCHORS` synced). `RelSpec` (pivot/widthFrac/anchor) is the only per-item placement model; items without one fall back to their category preset (`CATEGORY_OVERLAYS`). A RelSpec may carry a **per-pose override** (`perAnim`, 2026-09-17) — a complete spec scoped to one animation family, for a side sprite's own pivot on the turn or a one-eyed item's near eye; the base spec stays the front truth, and the **Sheet** tab draws every item on the pig at one pose for review. The legacy per-item `HAT_OVERLAYS` path, `compute_overlays.py`, and `hat_overlays.generated.ts` were removed 2026-09-12. Full workflow in `docs/placement-process.md`. (The old `item-anchor*`/`anchor-editor` tools were removed.)
