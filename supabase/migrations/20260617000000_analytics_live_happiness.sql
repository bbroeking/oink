-- Analytics overview, v3 — live happiness, per pig.
--
-- Happiness decays 0.5/hr since happiness_updated_at, clamped to [20,80] (this
-- mirrors public.happiness_now()). v1/v2 bucketed the *stored* value; this makes
-- the distribution use the LIVE decayed value and adds a `pigs` roster: every
-- (named) user's current happiness + band, happiest first. Aggregate/usernames
-- only; granted to anon.
...
as $$
  ...
  select jsonb_build_object(
    ...
    -- live (decayed) happiness distribution
    'happiness', (
      select jsonb_build_object(
        'sad',     count(*) filter (where live_happiness < 34),
        'neutral', count(*) filter (where live_happiness >= 34 and live_happiness < 67),
        'happy',   count(*) filter (where live_happiness >= 67),
        'avg',     round(coalesce(avg(live_happiness),0), 1)
      ) from real_profiles where happiness is not null
    ),

    -- every named pig's current happiness, happiest first
    'pigs', (
      select coalesce(jsonb_agg(t), '[]'::jsonb) from (
        select
          coalesce(nullif(username, ''), '(no name)') as username,
          round(live_happiness, 1) as happiness,
          case when live_happiness < 34 then 'sad'
               when live_happiness < 67 then 'neutral'
               else 'happy' end as band
        from real_profiles
        where happiness is not null
        order by live_happiness desc, username
      ) t
    ),
    ...
    'most_wasted', (
      select coalesce(jsonb_agg(t), '[]'::jsonb) from (
        select
          ...
          round(tickles_wasted_total::numeric / nullif(tickles_wasted_total + tickles_earned, 0) * 100) as pct
        from real_profiles
