-- Compress blessings + curses durations into a 3-6h window.
--
-- Was a wide spread: warm_tea/sluggish_snout = 1h, halo_kiss = 6h,
-- goblin_whisper = 4h, sun_beam = 24h, phantom_itch = 24h. The 1h
-- effects barely register before they fade; the 24h "next lucky pig"
-- / "next 3 taps" curses outlive a whole session and lose their bite
-- as a daily ritual moment.
--
-- New schedule (all in 3-6h):
--   warm_tea         3h   mild blessing → enough to feel a tap or two
--   sun_beam         4h   a sitting's worth of boosted lucky-pig chance
--   halo_kiss        6h   premium blessing, longest visible streak
--   sluggish_snout   3h   mild curse mirror
--   goblin_whisper   4h   mid curse mirror
--   phantom_itch     6h   premium curse — slips persist through the day
--
-- The instant kinds (bountiful_snouts, coin_pinch) stay NULL — they
-- have no duration concept. send_blessing + send_curse are otherwise
-- byte-identical to the 20260534 versions.
...
BEGIN
	...
	-- All timed effects now sit in the 3-6h window — see header.
	exp := CASE kind_today
		WHEN 'warm_tea'  THEN now() + interval '3 hours'
		WHEN 'sun_beam'  THEN now() + interval '4 hours'  -- boosted lucky-pig chance
...
BEGIN
	...
	exp := CASE kind_today
		WHEN 'sluggish_snout' THEN now() + interval '3 hours'
		WHEN 'goblin_whisper' THEN now() + interval '4 hours'
		WHEN 'phantom_itch'   THEN now() + interval '6 hours'
