-- Add cap + next_regen_seconds to barn_visit_status so the redesigned Visit &
-- Tickle screen's "YOUR TICKLES" bar can show avail/cap and a live refill timer.
-- Everything else unchanged from 20260605. (tickle_at_barn untouched.)
...
BEGIN
	...
	-- Seconds until the next tickle regenerates (null when at/over cap).
	v_next_regen := CASE
