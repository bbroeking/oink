-- home_stats(): additionally return 'regen_seconds' — the caller's TRUE
-- per-tickle regen period from regen_secs_for() (VIP x0.5, warm_tea x0.5,
-- sluggish_snout x2, alignment +/-10%, happiness +/-15%). The Barn's
-- "Next tickle" toast was hardcoding "+1 every hour", which lies the
-- moment any modifier is live. Additive key — old clients ignore it.
-- Body otherwise verbatim from 20260599 (latest home_stats def).
