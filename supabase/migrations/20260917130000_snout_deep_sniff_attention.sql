-- Snout Deep: the sniff budget — "so many free sniffs, then he starts to
-- notice" (founder, 2026-09-16). Spec §1.4 amended the same day.
--
-- The rule: the first 5 sniffs of a dig roll at the table's odds (in topsoil
-- that is truly free). Every sniff past the budget adds +1 (in 120ths) per
-- extra sniff to its threshold — the 6th sniff +1, the 7th +2 … — capped at
-- the layer's shove, so a sniff is never louder than a shove. Rubs and shoves
-- are untouched. Sniffs are counted over the whole dig, across layers, from
-- the action log, so client (utils/rooting.ts wakeThreshold / sniffAttention)
-- and server count the same sniffs. Mirror: constants/dig.ts
-- SNIFF_FREE_PER_DIG = 5, SNIFF_ATTENTION_STEP = 1 — __tests__/
-- snoutDeepServerParity.test.ts pins the two.
--
-- What lands here:
--   1. _snout_deep_wake_threshold(layer, verb, coop, prior_sniffs) — a 4-arg
--      overload over the 3-arg base (kept: harness 87 pins the base table).
--   2. _submit_rooting_deep_core — CARRY-LATEST-DEF: body carried VERBATIM
--      from 20260914090000_loose_pouch.sql (the latest definition; the
--      chain is 20260913060000 → 20260913120000 → 20260914090000) with ONE
--      change: the replay loop counts sniffs and passes the count to the
--      4-arg threshold. Nothing else in the dig changes.
--
-- Open digs at push time: a row opened before this lands replays under the
-- new rule on submit. Its 6th+ sniff was rolled on the phone at the old odds
-- and may now read as a wake on the server — the receipt would disagree
-- with the phone for that one dig. Push between feeding windows (the Eastern
-- gap 23:00–06:00) so no Snout Deep row is open.
--
-- Migrations are WRITTEN, NOT PUSHED — never `db push` without the founder's
-- explicit "go".

-- ── 1. The threshold, with attention ─────────────────────────────────────────
CREATE OR REPLACE FUNCTION public._snout_deep_wake_threshold(p_layer int, p_verb text, p_coop boolean, p_prior_sniffs int)
RETURNS int LANGUAGE sql IMMUTABLE
AS $function$
	SELECT CASE
		WHEN p_verb = 's' THEN LEAST(
			public._snout_deep_wake_threshold(p_layer, p_verb, p_coop)
				+ GREATEST(0, COALESCE(p_prior_sniffs, 0) + 1 - 5) * 1,
			public._snout_deep_wake_threshold(p_layer, 'h', p_coop))
		ELSE public._snout_deep_wake_threshold(p_layer, p_verb, p_coop)
	END;
$function$;
REVOKE ALL ON FUNCTION public._snout_deep_wake_threshold(int, text, boolean, int) FROM PUBLIC, anon, authenticated;

-- ── 2. The core submit — carried, the loop counts sniffs ─────────────────────
...
BEGIN
	...
	IF n > 0 THEN
		...
		FOR i IN 1..n LOOP
			...
			lyr := substr(e, 2, 1)::int;
			verb := substr(e, 1, 1);
			-- The sniff budget (20260917130000): the k-th sniff past the
			-- budget draws his attention. prior_sniffs counts the sniffs
			-- BEFORE this entry, the way the client's sniffCount(actions) does.
