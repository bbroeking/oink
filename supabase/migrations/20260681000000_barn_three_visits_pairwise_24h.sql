-- Barn visiting, redesigned (live exploit fix). The 20260676 rework DROPPED
-- the per-visitor daily cap ("the per-friend gate is now the real ceiling"),
-- but the per-friend gate only bounds farming ONE friend — across many friends
-- there was NO ceiling, so leaderboard tickles scaled with friend count (the
-- reported "I get way more tickles than Jen because I have more friends").
--
-- NEW MODEL (per the SKILL.md decision log, 2026-06-26):
--   • PAIRWISE COOLDOWN = 24h. Once you visit a friend's Barn, you can't visit
--     THAT friend again for 24 hours.
--   • VISIT BUDGET = 3 DISTINCT barns per rolling 3h window. You have to spread
--     your visits across DIFFERENT friends; the budget refills as old visits
--     age out of the 3h window.
--   • Both host AND visitor still get leaderboard credit (counter +
--     tickles_earned), unchanged.
--   • The per-visit random 3–7 tap cap is unchanged.
--
-- The budget refusal reuses the 'cooldown' error + the locked/next_at status
-- fields (as 20260661 did for the old daily cap), so the existing client shows
-- the countdown with no app change. Carried VERBATIM from 20260678 apart from
-- the cooldown constant, the new budget gate, and the status budget readout.
...
-- the client shows the right countdown. Carried from 20260678 apart from the
-- 24h cooldown + the budget branch; the visitor's tickle-bank block is unchanged.
