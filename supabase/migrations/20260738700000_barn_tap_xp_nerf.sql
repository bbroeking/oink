-- Barn tap XP nerf: +5 -> +2 per tap.
--
-- The +5 was tuned when a visitor got ~1 tap per friend per day; the barn
-- rework (3 visits -> 7 taps per pair per 24h, unbounded pairs) multiplied
-- the cadence without revisiting the XP, so the barn lane paid ~5x the
-- intended engaged ceiling (docs/xp-audit-2026-07.md: a 21-friend social
-- grinder banked ~530 XP/day from taps alone on season day 1). At +2 the
-- lane stays the best social earner without dwarfing everything else.
--
-- Carried VERBATIM from 20260705300000_barn_forage_truffle.sql (the newest
-- tickle_at_barn def); the ONLY change is the grant_season_xp amount.
...
BEGIN
	...
	PERFORM public.grant_season_xp(caller_id, 2);  -- XP NERF: was 5 (see header)
