-- The loose pouch carries down — only Tie it off banks a thing.
-- Spec: docs/dig-redesign/a-snout-deep-spec.md §1.5 / §1.6 (rewritten
-- 2026-09-14), §5.7 / §5.8 (the tally's "lost" and "kept" rows); the plan in
-- docs/dig-redesign/tie-it-off-plan.md §2 (the corrected storage strategy).
-- SKILL.md decision 2026-09-14 "The loose pouch carries down".
--
-- The founder's rule, verbatim: "A thing that is loose stays loose until the
-- player presses Tie it off. Descending to the next layer does not bank it.
-- The loose pouch rides down with the player and remains at stake on every
-- deeper layer; if he wakes at the root, everything loose from topsoil, the
-- mud and the root is lost together."
--
-- What lands here — _submit_rooting_deep_core CARRIED from 20260913120000
-- (the latest def; the carry-latest-def footgun applies). DIFF:
--   1. Two classes of thing, by kind (constants/dig.ts DIG_CONSUMABLE_KINDS /
--      DIG_COLLECTION_KINDS MUST match): a CONSUMABLE (boom · pouch · apple ·
--      shimmer · acorn · tea · scroll · charm) is loose until the tie; a
--      COLLECTION thing (junk · relic · furnishing · bow) is kept on reveal.
--   2. p_finds is the BANKED list: the truffles (as before) AND the
--      consumables the tie banked. A consumable in p_finds is accepted only
--      by kind, board-shaped, from a layer the dig reached — else bad_finds,
--      the way a wrong truffle is refused. The server does not trust the
--      client for what was lost: it derives `woke` from the log replay as it
--      always has, and ON A WAKE EVERY CONSUMABLE CLAIMED IS FORCED OUT OF
--      THE BANK AND LISTED LOST (nothing banks without a tie — a wake is the
--      absence of one), exactly as the current layer's truffle is forced out.
--   3. p_missed carries the lost consumables as well as the taken truffle;
--      they are listed on the receipt at 0 with `lost: true` and nothing is
--      granted or paid. On a tie a consumable in p_missed is a client bug
--      and is dropped (the tie banked everything loose).
--   4. p_things is the KEPT collection list: granted always (recorded on the
--      receipt's `things`, paid through their own paths in a later pass),
--      their tickles paid only when the dig tied; on a wake they ride the
--      receipt at 0 with `kept: true`. A consumable sent in p_things is
--      re-routed by kind into the banked / lost lane — the list it arrived in
--      never decides what it is.
--   5. The receipt gains `banked_things` and `lost_things`; `things` narrows
--      to the kept collection ids. tickles[] rows: the taken truffle (0,
--      lost), the lost consumables (0, lost), the banked truffles, the
--      banked consumables, the collection things (paid, or 0 + kept).
--   Idempotent per (uid, window_index) as before: the stored receipt
--   short-circuits a re-submit before any write.
--
-- Untouched: sync_rooting / close_open_rootings still tie a closed window at
-- its synced truffles only (the pouch is not synced yet — a follow-up).
--
