# Season 2 flip runbook — "The Great Hunger" go-live

The exact operational sequence from the **July 12 Season-1 finale cron** to **Season 2 being live**. Every step is grounded in a migration or flag that exists in this repo; anything not yet decided or not verifiable from the repo is marked **[OPEN QUESTION]**.

> **Read first.** Nearly every Season-2 migration carries a `HELD FOR REVIEW — push only on Brian's explicit "go"` header (`20260692`, `20260704100000`–`20260704600000`). Per `CLAUDE.md`, DB pushes are **never** autonomous. This runbook is the checklist for the human who runs `npx supabase db push` from Brian's machine, in order.
...
| `app_config.'season1_finale'` | `20260704400000_beta_rewards.sql:50` | global **false** | reveals the `SeasonEndModal` beta-founder recap |
...
| `run_judgement_day_season1()` | `20260704500000_judgement_day_beta_grants.sql:31` | cron | the Jul-12 finale engine (below) |
| cron `judgement-day-season-1` | `20260704500000:60` | `0 0 13 7 *` | **00:00 UTC Jul 13 = 8:00 PM ET Jul 12** |
| `finalize_season('season_1')` | latest def `20260704600000_finalize_season_ambiguity_fix.sql` | service-role only | grants S1 alignment finale titles + snouts, then **wipes `alignment_score` to 0** |
...
| Client S1 countdown | `utils/season.ts` `SEASON_1_END` | `2026-07-12` | display only; see `20260704500000` header |
...
1. **Push the Season-2 migration stack** in filename order (all currently HELD-FOR-REVIEW):
   ...
   - Any merged Season-2-queue migrations (e.g. the S2 battle pass `20260706*` from PR #16, the barn-forage truffle `20260704700000` from PR #17) once reviewed and merged.
   ...
   > **Battle-pass gap to note:** the Season-1 pass `ends_at` was extended to `2026-07-06` (`20260568000000_extend_season_1.sql:13`), but the finale cron fires ~6 days later (Jul 12/13). Between Jul 6 and the flip, `active_season()` returns **nothing** (no S2 `seasons` row exists in the repo yet — PR #16 adds `snout_season_2`). Decide whether that dead-pass window is acceptable or whether S2's `starts_at` should backfill it. **[OPEN QUESTION]**
...
3. **Ship the first Season-2 app build** (TestFlight → App Store; local build per `CLAUDE.md`). It **must** contain:
   ...
   - `utils/season.ts` `SEASON_1_END = 2026-07-12` (moved in the same commit as `20260704500000`).
   - The bundled war-spoils art (all 25 since build 101) so the Truffle Exchange / `SpoilsShowcase` render.
   - **[OPEN QUESTION]** confirm the S2 battle-pass client (PR #16) is in this build if the pass is meant to appear at flip.

---

## 2. The Jul-12 finale — mostly automatic

The cron `judgement-day-season-1` fires `run_judgement_day_season1()` at **00:00 UTC Jul 13 (8:00 PM ET Jul 12)**. In one transaction it (each step independently exception-guarded, `20260704500000:37`):

- **a.** `finalize_season('season_1')` — grants S1 alignment finale titles + snouts, then **wipes every `alignment_score` to 0** (the Season-2 slate). Idempotent per `season_key`.
...
```sql
SELECT jobname, schedule, command FROM cron.job WHERE jobname = 'judgement-day-season-1';
...
## 3. Flip Season 2 live

`finalize_season` and the finale reveal are **not** the same thing as turning on Season 2. The two boss/war flags default global-false and are **not** touched by the cron — a human flips them. Do this **after** confirming §2 landed:
...
- **Recap surface:** gated by `season1_finale`; `SeasonEndModal` reads `my_beta_reward()`. (PR #19 in the S2 queue adds a persistent re-entry icon so the recap can be reopened after its one-time auto-play.)
- **Idempotency:** `ON CONFLICT (user_id) DO NOTHING` — re-running never double-grants.

---

## 6. What must be in the first Season-2 app build (consolidated)
...
3. `utils/season.ts` `SEASON_1_END = 2026-07-12`.
4. Bundled war-spoils art (present since build 101) for the Truffle Exchange + `SpoilsShowcase`.
5. **[OPEN QUESTION]** the S2 battle-pass client (PR #16) and the Great Hunger story video (PR #18) if they're meant to be live at flip.
...
- [ ] `active_season()` returns the S2 pass **[OPEN QUESTION — depends on PR #16 merge + its `starts_at`]**.
...
- [ ] `alignment_score` is 0 for all (S1 slate wiped).
...
3. Is the S2 battle pass (PR #16) meant to be live at flip, and is its client in the first build? (§1.3, §6)
4. Are PR #18 (story video) / other S2-queue surfaces required in the first build, or fast-follow? (§6)
