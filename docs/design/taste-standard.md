...
- **2026-09-13 — A pig at rest breathes.** The "Indoors, the pig sits" entry above was half right: the standing idle *is* one drawing (`idle_2`/`idle_4` are the same pose; the pixel diff is edge jitter), but the Exterior does not hide it — Rosie has been a still on Home since the stance-continuity playback landed, and the seated rest's one 0.8 s squint in every 4 s reads as a still too on a 60 pt sprite. Measured on-device: the frame loop runs (ticks 0→3 at 2.5 fps) and the picture never changes. Ruling: rather than reopen the trot (build 179's four-frame idle, where the far hoof pops in and out every tick) the whole stage breathes — `breathe` in `utils/motionRecipes.ts`, a 3.2 s out→in→out sine on `PigStage`'s root (`BREATH_STRETCH` 1.03 in Y, `BREATH_SQUASH` 0.985 in X, lifted by half the stretch so the hooves hold the floor). The stage breathes, not the sprite, so a worn hat rises with her; it runs only at rest (`idle`/`sit` — a reaction or a mood carries its own motion), never on a frozen pose (list avatars, item previews), never under Rive, and Reduce Motion leaves her still. The blink stays on top of the breath in the room. Rule going forward: **the two-distinct-drawings rule is satisfied by procedure as well as art — a rest that cannot change its picture must change its shape.** Serves **craft** (rule 6: the pig is alive wherever she stands).

- **2026-09-14 — A row's actions may fold, but never into the row's height.** Same-day amendment: the morning's "four verbs, four cells" bar was built, then set aside for an anchored panel behind a "…" trigger. The standing rule: a list row is one shape at every state. Actions are either a rail of one or two round doors, or a **panel that slides over the row's own box** — absolute, clipped at the card edge, `cream2` on `RADII.md`, five equal icon-over-label cells (`label` role, one line, shrinking to fit; no state line on any tier — it cost the row 20pt and the curse's arm shows as a heavy border plus the label flipping to "Again"; `layoutBreakpoints.ts` asserts the five cells clear `TAP_MIN` at 375pt and the row floor stays at 70pt). A footer tray that grows the row, a chevron that reveals a second layout, or a menu that navigates away are all out. The trigger is the `more` glyph in a 44pt frame and does not swap art when open — the panel beside it is the open state. The panel arrives on a short, clamped spring with a fade (`ACTION_PANEL_TRAVEL`, shorter than the trigger clearance, so nothing is ever clipped and nothing bounces). Serves **craft** (the list never reflows under the thumb) and rule 6 (a spring that lands, not a linear fade).

- **2026-09-14 — A Barn is a slower room, and nobody in it stands still.** Two fixes to how pigs rest in the Barn. First, the visitor's own pig in a friend's Barn was mounted `active={!me}` — frozen while the host moved, in the yard and in the room alike — with no reason recorded; the freeze is gone, so both pigs run the same rest (idle in the yard, `sit` in the room) and the same breath. Second, the Barn now declares its own pace: `PigRestTempoProvider` (beside `PigRestingPoseProvider`) at the root of the interior route and the visit modal carries `PIG_BARN_REST_TEMPO` (0.8), and every rest loop and breath inside runs at four-fifths speed — the 1.6 s idle becomes 2 s, the 4 s seated blink 5 s, the 3.2 s breath 4 s. Reactions are not scaled: a tickle answers at the same speed everywhere, because the answer is the game's, not the room's. Rule going forward: **a surface declares how its pigs rest — pose and pace — once, at its root; a pig never decides by itself which room it is in, and no pig on a surface is exempt from that surface's rest.** Serves **Connect** (a visit is two pigs in a room, and both must be alive) and **craft** (rule 6, and the Barn's calm is a tempo, not a stillness).

- **2026-09-14 — The Friends list says what is LEFT, not what things are called; and a sheet's cap must reach the thing that scrolls.** Three rulings from one pass on the Friends tab. (1) **The list's header is one quiet line.** The ritual strip had grown into two art chips carrying the day's ritual NAMES and two captions of copy under them ("Glimmer Truffle · 3 left" over "Their pig floats…") — a section nobody asked for, above the rows they came to see. What the list needs to say is the allowance: `Three glimmer left · three truffle left`, one `kicker` line, counts in words to ten, **"No"/"no" at zero and nothing at all while the allowance is unknown** (an unknown is never a placeholder that fills in under the thumb). Each half is its own door into the ritual's explain sheet, reaching `TAP_MIN` through `hitSlop` rather than by inflating the line. Names, art and effects stay where a player asked for them: the sheet, the row's action panel, the row's cast notice. (2) **One step, three times.** Between the crown's rule and the first friend sat four stacked breaths (nav margin, crown breath, panel `marginTop`, segment margin, strip margin). The panel starts flush — the `PageHeader` already breathes — and segment → count line → first row is `SPACE.sm` each, the last one spent by the list's own `gap`. (3) **A height cap that cannot reach the scroller is a clip.** `Sheet` capped the panel at 85 % of the window, but `flexShrink: 1` alone leaves a flex item's automatic minimum size at its content height, so a long body (a friend's profile) measured full and pushed the Report · Block row past the panel's floor and off a short screen. Every link from the cap down to the one `ScrollView` now carries `flexShrink: 1` **and** `minHeight: 0`; the body bounces again; and the home-indicator safety is spent by whatever is LAST — the pinned footer when there is one, the scroll content when there isn't, where it travels with the last row instead of sitting under it as a fixed band (amends 2026-09-13). Same pass: the action cells stopped shrinking type to fit. Measured against the 39.8 pt content box a 375 pt phone gives a cell, `kickerPillSm`'s 1.6 pt tracking cost more than the smaller glyph saved — CURSE, AGAIN, UNPIN and PROFILE all overflowed and the shrink rescued only the first three — so both tiers speak in `label`, where all five words fit at full size. Rule going forward: **a label that needs to shrink is a label in the wrong role, or a word in the wrong cell.** Serves **Connect** (the list is friends, not a poster for today's rituals) and **craft** (rules 1 and 2, and a panel that ends where its content ends).

- **2026-09-14 — Two pigs on a visit face each other, and the thing you cannot tickle wears the reason.** The visit diorama put the guest back-left/small and the host front-right/large, both drawn from the same front-facing sprite — two pigs staring at the camera. Now one ground line, the guest left as drawn, the host right and **mirrored** (`PigStage facing`, a `scaleX: -1` composed on the stage wrapper beside the ritual flip, so every anchored cosmetic rides along and raster and Rive agree); Rosie's tail and head-tilt do the rest. The chip under the host pig counts the visit's taps and, when the cap lands, **becomes** the "tickled out" tag (`zzz`) — one slot that changes meaning rather than a second piece of chrome, and the toast is only the moment. The nametag, chip, wish bubble, floats and ground shadow sit outside the stage and never mirror. The Satchel's strip is **tap, not drag** — a lifted find that reads "give this" is the same decision with one motion fewer, and no pan has to fight the pigs' presses on a `box-none` layer. Find art is one keyed ImageGen sheet sliced into twelve painted glyphs in the dig marks' style; keepsakes are drawn from the finds themselves, so no art is owed for a reward. Serves **Connect** (the visit reads as two pigs meeting) and **craft** (a state the player can see, not remember).

- **2026-09-15 — A pair faces each other with a drawn turn, and a stage never scales its actors twice.** The 2026-09-14 mirror was a transform standing in for a drawing; on the Inside visit it read as two pigs at the camera, each about 60pt tall, in a room whose painted shelf and wardrobe outweighed them. Rulings: (1) **a pose the player is meant to read is drawn, not transformed** — the three-quarter `face` / `face_sit` families (Codex ImageGen, magenta-keyed, sliced at one shared scale so the seated row reads as sitting, not shrinking; `scripts/pig-tweens/slice_face_sheet.py`); the mirror survives only as the left/right of one drawing. (2) **One owner of size per surface.** `HabitatScene` sizes the pigs it stages (`PIG_SPOTS`) and takes the 300pt canvas at 1:1; a caller never pre-shrinks what a stage will scale. Two nested scales were how a 172pt host became 87. (3) **The base room is empty**: furniture painted into a background is décor the player did not choose and cannot move, and it fights the six decorating spots — `warm_plank_barn` is repainted with walls, beams, window light and floor only. All three rooms are repainted empty; the "Shelf" spot draws its own plank (`HABITAT_CHROME_ASSETS.shelfPlank`, its top edge on the set-down object's own floor, under it in z) — the shelf is the spot's, not the wallpaper's. A one-plank room variant survives in `docs/reviews/pig-facing-2026-09-15/room_shelf/` unused. Serves **craft** (rule 6, and a scene whose every object is someone's decision).
- **2026-09-15 — The cast moment is a bubble, not a card.** A landed blessing or curse used to be told twice over: the friend row grew its "blessed · Cloud Nine" notice AND the Inbox bless-back raised a success toast — a card at the top of the screen, the same shape every outcome in the app wears. Now a landed cast is one moment, `components/ui/RitualBubble.tsx`: the ritual's own painted sticker sealed in a soap bubble that surfaces from under the bottom edge, climbs the whole screen swaying, and pops just under the safe area — the same top line a Toast sits on — while a paper tag hangs under it (*Cloud Nine → Bandit*). A blessing is buoyant (`MOTION.ritualRiseBless` = the toast dwell, sparkles let go behind it); a curse is a beat heavier (`MOTION.ritualRiseCurse`, bigger tilt, drips fall). Rulings: (1) **the record and the moment are different things** — the row's notice stays and is the record; the bubble never carries a dismiss, a title, or a second sentence. (2) **Refusals keep the toast**: a refusal has nothing to celebrate and needs its reason read. (3) **The profile sheet's `RitualPicker` keeps its in-sheet "sent" beat** — the sheet is a native Modal, which paints over a root host, so a bubble fired from there would rise unseen behind it. The bubble fires from the friend-row doors and the Inbox bless-back only. (4) Reduce Motion: no travel, no loops — a cross-fade at mid-screen held for `MOTION.beat`; the VoiceOver announcement is not motion and always fires. (5) The host is a `pointerEvents="none"` layer under the Toast's z — the build-99 lesson, pinned by test. (6) The native driver takes no `easing` inside an `interpolate` (a red-box, and the travel stalls): the curve lives on the master value and every stop is read through it. Watch it without spending a cast: `ticklethepig://ritual-bubble-preview` (`?motion=reduced`). Serves **craft** (rule 6: a send with weight, warmth and a pop — never a linear fade that could belong to any app) and **Connect** (the cast is felt as a thing leaving your hands, not read as a receipt).
- **2026-09-15 — The friend row sits straight, marks its rituals in art, and its list bleeds.** Three rulings from one screenshot (a friend with both marks ran the meta line to four rows, the card's right edge looked cut, and the row leaned). (1) **A landed ritual is a mark, not a sentence**: `RitualCastNotice` is the cast ritual's own art on a 24pt disc in the door's tint (`RITUAL_DOOR[mode].fill` — sun / the curse pair's sage), both marks in one trailing cluster so they wrap together; the words move to the accessibility label. The art is the app's existing "this landed" vocabulary (the settled door, the Inbox row, the bubble); sparkle/cloud stay the resting-door marks. (2) **A dense row does not lean**: `ListRow tilt={false}` on the friend row. The scrapbook tilt (`ROW_TILTS`) stays on the one-line lists — rankings, season tiers, the Inbox — where it reads as a pile of stickers rather than as a clipped card. (3) **A list inside a padded wrapper bleeds**: the Friends `FlatList` clipped at its own edge, exactly where each row's `SHADOW_SM` falls; it now runs `marginHorizontal: -LIST_BLEED` with its content padded back by `LIST_BLEED` (the while-away recap's pattern), so the rows keep the `PAGE_PAD` inset and the shadow is whole. The Inbox pads inside its scroll and never clipped. Serves **craft** (rule 1: tokens, and a row whose every mark is legible) and **Connect** (the friend list is read many times a day; it must read at a glance).
- **2026-09-15 — A toast shows over whatever is up, including a native Modal.** The one `ToastHost` sat at app root, and a native `<Modal>` (`Ceremony` for a visit, `SlideUpSheet` for the profile sheet) paints over the root — so every toast fired inside the visit (the wrong-find bounce, "hoping for something else now", "All tickled out") landed unseen. Rulings: (1) hosts register in a stack and `showToast` addresses the last one mounted, never a broadcast; unmount removes by identity so a sheet closing under a visit hands nothing to the wrong host. (2) `Ceremony` and `SlideUpSheet`'s native branch mount a host of their own, last in their tree. (3) The toast wrap is the topmost transient wherever its host sits — `TOAST_Z` 200, because the visit's root fills its Modal at 100 and a host at 50 painted under the barn (found on device, iteration 1). Verified in two identical passes on the simulator: bounce, stale-wish `wrong_find` with the bubble re-rendering, tickled-out, root hand-back after the visit and the sheet, cold relaunch, Reduce Motion (`docs/reviews/visit-toasts-2026-09-15/`). Serves **craft** (rule: the world responds *now* — and visibly).
- **2026-09-15 — On the visit, a toast sits under the chrome, not over the tallies (placement B).** Five placements were drawn on the real screen (the app-wide line over the tallies; under the header; above the strip; a callout from the tapped tile; the pig's own bubble) and B was chosen. Rulings: (1) `ToastHost` takes an explicit `top`; the default stays the app-wide line. (2) A scene with chrome of its own mounts its own host on its own line — the visit's `VISIT_TOAST_LINE` = `VISIT_CHROME_H` + `STATUS_ROW_GAP`, every term a named part of the chrome the visit already lays out (`components/visit/chrome.ts`). (3) A Modal primitive's fallback host mounts BEFORE its children: effects run in tree order, and the scene's host must register last to take the calls. Serves **craft** (the tallies are the visit's score; a message never hides the score).
- **2026-09-15 — A turned pig wears the item's side sprite.** The three-quarter `face` / `face_sit` families turned the pig but not what it wore: the front sprite sat on a turned head like a cut-out on a photo. Rulings: (1) an item may carry a SIDE sprite (`assets/images/hats/side/<id>.png`, registry `constants/hat_side.generated.ts`, rebuild-all from the directory) and `PigStage.resolveSlot` takes it on the face families only — reactions and moods play front frames and keep the front sprite; an item without side art keeps its front sprite, so nothing regresses. (2) Side art is REPAINTED from the item's own front sprite at Rosie's `face_1` camera by the Codex ImageGen lane (`tools/gen_side_items.py`: front art + face frame as references, fidelity-first prompt, camera the only change), keyed by flood fill from the corners plus a hole pass for checkerboard/magenta so a glossy white highlight inside the ink survives. (3) Placement reuses the item's front RelSpec on the face families' own anchors; where a side sprite wants its own pivot, that is a placement-studio pass, not new art. First six: acorn_bow, astronaut, aviator_sunglasses, cowboy, tophat, wizard. Dev fixture: `ticklethepig://pig-side-preview` (Front/Side, Standing/Sitting, items, pigs, and the visit's tickle reaction). Serves **Collect** (what you wear turns with you) and **craft**.
- **2026-09-15 — Verification is a contact sheet of the real renderer.** `ticklethepig://pig-angle-audit` lays every worn cosmetic (~150) out as a row of frozen `PigStage` cells — Front, Front sit, Turn R, Turn L (the visit's mirror; lettering and asymmetric art flip), Turn L sit, plus opt-in reaction columns — filtered by pig, slot, side-art-only and flagged-only. A cell opens the full stage with a frame scrubber; a view can be FLAGGED with a note (local, `Copy report` → JSON on the clipboard for an issue or the regen studio). Ruling: item art is judged on the shipping renderer at every drawn angle, never on a standalone sprite — a front-only held item that reads fine face-on is the class of thing the sheet exists to catch. Serves **craft**.
- **2026-09-16 — What the dig put in your bag is drawn, and it is the tally's last beat.** The Satchel roll rode the dig receipt as one grey hand line above the count (*"your Satchel got heavier: a river pebble."*), filled in whenever the server's receipt landed — mid roll-up, above the number the eye was on, in the tone the sheet uses for asides. Players finished digs without knowing their bag had changed. Rulings: (1) **a thing you now own is shown as the thing**, never named in passing — the bag glyph, each find on the same paper tile the Satchel's bag row draws it on (`components/satchel/FindTile.tsx`, shared by both), a kicker pill and one hand line under them (*"a river pebble and an old key · 3 of 6 finds"*); a find the full bag turned away sits on a **ghost tile** — cream, dashed, silhouette — the catalog's never-carried grammar, because it is not yours. (2) **Tickles first, things second**: the bag is one more slot in the tally's landing after the last ledger row, a stagger later, and its finds drop half a row apart; a roll the server names after the rows are down lands on its own beat rather than appearing in place; the hurry tap lands it all; under Reduce Motion it is simply there. (3) The block is one accessibility element reading the old sentence — the sentence was never the problem, its placement was. Watch it without a Feeding: `ticklethepig://snout-deep-preview?satchel=2|full|none|late`. Serves **Collect** (the bag is what you collect; the receipt is where you learn it grew) and **Connect** (a find you noticed is a find you carry to a friend).
- **2026-09-16 — The Shop becomes a place: a storefront scene, with the grid kept underneath.** Chosen over Drop First (order) and The Neighbors' Shop (social) on a three-direction canvas; the Storefront keeps the card grammar's states (rarity coaster + dot, sage owned check, gold lock, the price as the action's face, the same preview sheet on tap) and repaints the container: a plank wall, a chalkboard that says when the shelves restock, three wooden shelves, a members' shelf under the Slop Club sign, a counter with a shopkeep. Rulings: (1) **the Trough is an object, not a section** — a wooden trough by the counter holding the sounder's open drives as notched rows, replacing the accordion; (2) **the sounder stands at the counter** wearing today's buys, tagged in the hand voice, absent when they bought nothing; (3) **Closet and Furnish hang as signs** so the counter front belongs to the pigs; (4) **a yard trough follows the mound's rule** on Home — steady state is the object alone, a tap unfolds one tag, only the tag opens the sheet, and the fan carries the verb; (5) the scene ships behind the 2-col grid as its Reduce-Motion / VoiceOver fallback, and the wall, shelves, counter and shopkeep are painted at 3× the tallest phone in Rosie's sticker hand. Canvas: `docs/design/claude-design/shop-2026-09-16/`. Serves **Collect** (a shop you walk into) and **Connect** (your herd is in the room).
