...
- **2026-09-14 — A row that owns four verbs shows four cells.** The friend row's collapsed tray hid its actions behind a chevron and compacted the identity when open, so the same row had two shapes and the pig floated against a five-line stack. Ruling: a list row's actions are either a rail of doors (one or two, round, glyph-only, named by a strip above the list) or an **always-visible action bar** under the identity (three or four equal `Sticker` cells, `RADII.md`, no shadow, icon over `label`, the one-line state as a kicker on regular phones and in the accessibility hint on narrow ones — `constants/layoutBreakpoints.ts`). Never a tray: a disclosure that changes the row's shape is a second layout to get wrong, and an action you cannot see is an action you will not take. The identity keeps ONE meta line (glyph + count · streak tag · wears) and one herd line so the avatar sits level with the name; a pinned row wears a masking-tape tag (`WHIMSY.tape` / `tapeEdge`, `TILT.tape`), not a glyph swap. Serves **craft** (one shape per row, nothing hidden) and rule 1 (the tape, the cells and the tiers are tokens, not new chrome).
