...
function seedSession(key: string): Session {
  const session: Session = { wallet: { motes: 40, revision: 1 }, receipts: new Map(), order: [], next: 0, resourceBalance: 0, unlocked: false, activeUntil: null };
...
export function createMoteGameAcceptanceClient(scenario: string | string[] | undefined, name: string | string[] | undefined): MoteGameAcceptanceClient | null {
  ...
  const session = sessions.get(key) ?? seedSession(key);
