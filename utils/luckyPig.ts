...
//
// NO RITUAL TOUCHES LUCK any more (weekday rituals, 2026-09-14): the
// sun_beam / glimmer_truffle boost and its consume-on-first-lucky clear are
// gone. The only inputs left are the first-time guarantee and the clock.
...
export function rollLucky({
	...
	const chance = withinBoost ? LUCKY_TRIGGER_CHANCE_BOOST : LUCKY_TRIGGER_CHANCE;
	if (random() < chance) {
		return { triggerNow: true, reason: withinBoost ? "boost" : "steady" };
	}
