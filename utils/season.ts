// Season 0 schedule — the single source of truth for WHEN each
// Season 0 (greedy/generous era) feature went live.
//
...
export type SeasonFeature = keyof typeof SEASON_0_UNLOCKS;
...
export function featureUnlocked(
	...
	return todayISO(now) >= SEASON_0_UNLOCKS[feature];
}
...
export function seasonWeek(now: Date = new Date()): number {
	const start = Date.parse(SEASON_0_START + "T00:00:00Z");
	const diff = now.getTime() - start;
	return Math.max(1, Math.floor(diff / (7 * 86_400_000)) + 1);
}

// True while Season 0 is running (between start and end, inclusive).
